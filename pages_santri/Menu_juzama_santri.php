...
if ($keyword != "") {
    ...
    if ($category == 1) { 
        // Nama
        $query = "SELECT * FROM juzama WHERE nama LIKE '%$keyword%' ORDER BY id DESC";

    } elseif ($category == 2) { 
        // Kelas
        $query = "SELECT * FROM juzama WHERE kelas LIKE '%$keyword%' ORDER BY id DESC";

    } elseif ($category == 3) { 
        // Jilid
        $query = "SELECT * FROM juzama WHERE jilid LIKE '%$keyword%' ORDER BY id DESC";
    } elseif ($category == 4) { 
        // Halaman
        $query = "SELECT * FROM juzama WHERE jilid LIKE '%$keyword%' ORDER BY id DESC";
        ...
        // Status
        $query = "SELECT * FROM juzama WHERE status LIKE '%$keyword%' ORDER BY id DESC";
    ...
    $query = "SELECT * FROM juzama ORDER BY id DESC";
}
...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="tambah_data_juzama_admin.php" class="btn-add">
    ...
    <select name="category" class="form-select" style="max-width:200px;">
        ...
        <a href="?p=data_juzama_admin" class="btn btn-secondary">Reset</a>
    </form>
...
?>
    <tr>
        ...
        <td>
            <a href="edit_data_juzama_admin.php?id=<?= $row['id']; ?>" class="btn-action-edit btn-sm me-1" title="Edit Data">
                <i class="fa fa-edit"></i>
            </a>
            <a href="hapus_data_juzama_admin.php?id=<?= $row['id']; ?>" class="btn-action-delete btn-sm" title="Hapus Data" onclick="return confirm('Yakin ingin menghapus data ini?');">
